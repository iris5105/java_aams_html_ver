...
import com.kfp.aams.domain.dailyadvisory.service.Scm1pgService;
...
public class Scm1pgController {

    private final Scm1pgService scm1pgService;
    private final MenuService menuService;

    @GetMapping({"/views/w_scm1pg", "/views/dailyadvisory/w_scm1pg"})
    public String viewScm1pg(@AuthenticationPrincipal Object principalObj,
        ...
        var menuDto = menuService.getMenuByPgmId("W_SCM1PG");
        if (menuDto == null) {
            menuDto = menuService.getMenuByPgmId("w_scm1pg");
        }
        ...
        return "views/dailyadvisory/w_scm1pg";
    }

    @GetMapping("/api/daily/scm1pg/master")
    @ResponseBody
    public ResponseEntity<List<Scm1pgMasterDto>> getMasterList(
        ...
        List<Scm1pgMasterDto> list = scm1pgService.getMasterList(corpGr, ymd);
        return ResponseEntity.ok(list);
    }

    @GetMapping("/api/daily/scm1pg/detail")
    @ResponseBody
    public ResponseEntity<List<Scm1pgDetailDto>> getDetailList(
            @RequestParam(name = "corpGr", required = false) String paramCorpGr,
            @RequestParam(name = "jmCd") String jmCd,
            @AuthenticationPrincipal Object principalObj,
            @CookieValue(name = "savedCorpGr", required = false) String cookieCorpGr1,
            @CookieValue(name = "corpGr", required = false) String cookieCorpGr2) {
        ...
        List<Scm1pgDetailDto> list = scm1pgService.getDetailList(corpGr, jmCd);
        return ResponseEntity.ok(list);
    }

    @PostMapping("/api/daily/scm1pg/save")
    @ResponseBody
    public ResponseEntity<Map<String, Object>> saveDetail(
            @RequestBody Scm1pgSaveDto saveDto,
            @AuthenticationPrincipal Object principalObj,
            @CookieValue(name = "savedCorpGr", required = false) String cookieCorpGr1,
            @CookieValue(name = "corpGr", required = false) String cookieCorpGr2) {
        ...
        try {
            scm1pgService.saveDetail(saveDto);
            return ResponseEntity.ok(Map.of("success", true, "message", "저장이 완료되었습니다."));
        } catch (Exception e) {
            log.error("Error saving scm1pg detail:", e);
