forward
global type fw_n_animation_bak from nonvisualobject
end type
end forward

global type fw_n_animation_bak from nonvisualobject autoinstantiate
...
event oue_slide_up();IF IsValid(idrag_target) THEN
	...
	ELSE
		Yield ( )
		IF IsValid(idrag_target) THEN idrag_target.y -= ii_udterm
	END IF
ELSE
	in_time.stop()
END IF
...
public subroutine of_push4slidedown (dragobject adrg_target, integer ai_y_start, integer ai_y_end, double ad_time);//Decimal		ldc_time
//
//idrag_target	= adrg_target
//ii_slide_start		= ai_y_start * -1
//ii_slide_end		= ai_y_end
//ii_movemode	= MOVEDOWN
//
//idc_slidetime	= ad_time
//ldc_time	= idc_slidetime / ii_secondframe
//ii_udterm	= 2 //( ai_y_end - ai_y_start) / (idc_slidetime * ii_secondframe)
//
...
on fw_n_animation_bak.create
call super::create
TriggerEvent( this, "constructor" )
end on

on fw_n_animation_bak.destroy
