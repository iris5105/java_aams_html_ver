forward
global type fw_n_style_cond from fw_n_style
end type
end forward

global type fw_n_style_cond from fw_n_style
end type
global fw_n_style_cond fw_n_style_cond
...
string		ls_visible, ls_objtype, ls_editstyle, ls_border
string		ls_objbgcolor, ls_band, ls_bandheight, ls_bandcolor
string		ls_tabseq, ls_bordersyn
			...
			choose case ls_objtype
				case 'text'
					choose case idw_target.describe(isobjects[i] + ".text")
						case '~~', '/', '-', '.'
							ls_createsyntax += 'create rectangle(band=' + ls_band + ' x="' + string(ll_cellxpos) + '" y="' + string(ll_cellypos) + '" height="' + string(ll_cellheight) + '"' + ' width="' + string(ll_cellwidth) + '" name='  + isobjects[i] + '_rect visible="' + ls_visible + '" brush.hatch="6" brush.color="' + string(gnv_vari.setcondbackcolor) + '"' +  &
														' pen.style="0" pen.width="' + string(PixelsToUnits(1, XPixelsToUnits!)) + '" pen.color="1073741824~t' + string(gnv_vari.setcondbackcolor) + '" background.mode="1" background.color="553648127" )~r~n'
						case else
							ls_createsyntax += 'create rectangle(band=' + ls_band + ' x="' + string(ll_cellxpos) + '" y="' + string(ll_cellypos) + '" height="' + string(ll_cellheight) + '"' + ' width="' + string(ll_cellwidth) + '" name='  + isobjects[i] + '_rect visible="' + ls_visible + '" brush.hatch="6" brush.color="' + string(gnv_vari.setcondbackcolor) + '"' +  &
														' pen.style="0" pen.width="' + string(PixelsToUnits(1, XPixelsToUnits!)) + '" pen.color="1073741824~t' + string(gnv_vari.setcondbackcolor) + '" background.mode="0" background.color="553648127" )~r~n'
					end choose
...
ls_modifysyntax += 'datawindow.color="' + string(gnv_vari.setcondbackcolor) + '"'
...
public function string of_thisname ();return 'fw_n_style_cond'

end function

on fw_n_style_cond.create
call super::create
end on

on fw_n_style_cond.destroy
call super::destroy
end on
