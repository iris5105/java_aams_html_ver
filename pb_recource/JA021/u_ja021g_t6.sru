forward
global type u_ja021g_t6 from utt_listdetail
end type
end forward
...
global u_ja021g_t6 u_ja021g_t6

on u_ja021g_t6.create
call super::create
end on

on u_ja021g_t6.destroy
call super::destroy
end on

type ln_temptop from utt_listdetail`ln_temptop within u_ja021g_t6
end type

type ln_tempstart from utt_listdetail`ln_tempstart within u_ja021g_t6
end type

type ln_templeft from utt_listdetail`ln_templeft within u_ja021g_t6
end type

type ln_cond_start from utt_listdetail`ln_cond_start within u_ja021g_t6
end type

type ln_tempright from utt_listdetail`ln_tempright within u_ja021g_t6
end type

type ln_cond1_yline from utt_listdetail`ln_cond1_yline within u_ja021g_t6
end type

type ln_dw1_yline from utt_listdetail`ln_dw1_yline within u_ja021g_t6
end type

type ln_tempbutton from utt_listdetail`ln_tempbutton within u_ja021g_t6
end type

type dw_pagelist from utt_listdetail`dw_pagelist within u_ja021g_t6
string dataobject = "d_ja021g_t61"
...
CHOOSE CASE dwo.name
   CASE "tr_ymd"
      IF datetime (date (MidA (data,1,10)))<iu_wpage.idt_workdate Then
         RETURN uf_itemerr (row, 'tr_ymd', '액면분할(병합)일이 작업일보다 같거나 커야 합니다.')
      End IF
...
event dw_pagelist::ue_deletestart;call super::ue_deletestart;IF Object.tr_ymd [tRow]<iu_wpage.idt_workdate  Then
   f_messageBox ('ERR', '액면분할(병합)일이 지난 자료는 삭제 할 수 없습니다.')
   RETURN 1
End IF
RETURN 0
end event
...
type dw_pagedetail from utt_listdetail`dw_pagedetail within u_ja021g_t6
...
type st_move from utt_listdetail`st_move within u_ja021g_t6
end type
