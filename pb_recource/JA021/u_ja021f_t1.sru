forward
global type u_ja021f_t1 from utt_listdetail
end type
end forward

global type u_ja021f_t1 from utt_listdetail
end type
global u_ja021f_t1 u_ja021f_t1

on u_ja021f_t1.create
call super::create
end on

on u_ja021f_t1.destroy
call super::destroy
end on

type ln_temptop from utt_listdetail`ln_temptop within u_ja021f_t1
end type

type ln_tempstart from utt_listdetail`ln_tempstart within u_ja021f_t1
end type

type ln_templeft from utt_listdetail`ln_templeft within u_ja021f_t1
end type

type ln_cond_start from utt_listdetail`ln_cond_start within u_ja021f_t1
end type

type ln_tempright from utt_listdetail`ln_tempright within u_ja021f_t1
end type

type ln_cond1_yline from utt_listdetail`ln_cond1_yline within u_ja021f_t1
end type

type ln_dw1_yline from utt_listdetail`ln_dw1_yline within u_ja021f_t1
end type

type ln_tempbutton from utt_listdetail`ln_tempbutton within u_ja021f_t1
end type

type dw_pagelist from utt_listdetail`dw_pagelist within u_ja021f_t1
string dataobject = "d_ja021f_t11"
...
uf_setColumn ('tr_cd', 'G31')
uf_setColumn ('jeungj_mth', '1')
...
event dw_pagelist::ue_deletestart;call super::ue_deletestart;IF Object.cheng_ymd [tRow]<iu_wpage.idt_workdate  Then
   f_messageBox ('ERR', '청약일이 지난 자료는 삭제 할 수 없습니다.')
   RETURN 1
End IF
RETURN 0
end event
...
type dw_pagedetail from utt_listdetail`dw_pagedetail within u_ja021f_t1
string dataobject = "d_ja021f_t12"
...
CHOOSE CASE dwo.name
   CASE "baej_jusu"
      Object.chui_aek[row] = dec(data) * Object.balh_ga[row]
      Object.upd_user[row]= 'icam'
END CHOOSE


end event

type st_move from utt_listdetail`st_move within u_ja021f_t1
end type
