forward
global type fw_n_cht4json from nonvisualobject
end type
type json_pair from structure within fw_n_cht4json
end type
end forward
...
global fw_n_cht4json fw_n_cht4json
...
private subroutine of_buildtree (fw_n_cht4json ajson, long handle)
private function string of_getformat (fw_n_cht4json ajson, string ident)
private function integer of_readobject (fw_n_cht4json vjson, integer alevel)
end prototypes
...
//"fw_n_cht4json.getattribute: attribute "+string(itemname,"[general]")+" does not exists!")
end subroutine
...
public function any addarrayitem (string arrayname);any jsons[]
json_pair pair
integer index

fw_n_cht4json ljson
ljson = create fw_n_cht4json
...
public function any addobject (string objname);fw_n_cht4json ljson
json_pair pair
integer index
...
for li = 1 to upperbound(larray)
	...
	if classname(la) = 'fw_n_cht4json' then
		of_buildtree(la, h)
	elseif classname(la)='any' then
...
private subroutine of_buildtree (fw_n_cht4json ajson, long handle);string ls
integer li,lj, h, pcx, h1
treeviewitem tvi
any la, larray[]
...
private function string of_getformat (fw_n_cht4json ajson, string ident);string ls, ls1, ls_return, ls_ident, ls_rtn
integer li,lj, li_max, lj_max
any la, larray[]
...
for li = 1 to li_max
	la = ajson.pairs[li].value
	choose case ajson.pairs[li].name
		case 'tooltips'
			ls = '"' + ajson.pairs[li].name + '": {"enabled": '
		case 'tooltipsrangegb'
			// skip
		case else
			ls = '"' + ajson.pairs[li].name + '": '
	end choose
	...
	elseif classname(la)='boolean' then
		choose case ajson.pairs[li].name			
			case 'tooltips'
				if la = true then
					if ibtooltipsrange = true then
						ls = ls + 'true,"mode": "index","callbacks": {"enabled": true,"mode": "index"}}'
					else
						ls = ls + 'true,"callbacks": {"enabled": true,"mode": "index"}}'
					end if
				else
					if ibtooltipsrange = true then
						ls = ls + 'false,"mode": "index","callbacks": {"enabled": false,"mode": "index"}}'
					else
						ls = ls + 'false,"callbacks": {"enabled": false,"mode": "index"}}'
					end if
				end if	
			case 'tooltipsrangegb'
				// skip
...
on fw_n_cht4json.create
call super::create
TriggerEvent( this, "constructor" )
end on

on fw_n_cht4json.destroy
