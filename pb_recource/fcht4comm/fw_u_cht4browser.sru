forward
global type fw_u_cht4browser from olecustomcontrol
end type
end forward
...
string binarykey = "fw_u_cht4browser.udo"
integer textsize = -9
integer weight = 400
...
global fw_u_cht4browser fw_u_cht4browser

type variables
constant long TIMEOUT = 1000 // mSecs
...
public:
	...
	boolean	A--------------------------------empty	/* empty Object */
	boolean	iberrorissue		= false
...
public function integer of_gethtmlstatus ()
end prototypes
...
public function string of_thisname ();return 'fw_u_cht4browser'
...
public function string of_getreadystate ();// "interactive", "complete"
if isnull(this.object.document) then
	//messagebox('Notice', '웹페이지가 로딩되지 않았습니다')
	iberrorissue = true
	return ''
else
...
public function integer of_gethtmlstatus ();if iberrorissue = true then
	return -1
end if
string	ls_htmlstatus
long	ll_start
...
ll_start = cpu()
ls_htmlstatus = this.of_execscript("gethtmlstatus();")
if isnull(ls_htmlstatus) then ls_htmlstatus = "empty"
if not(ls_htmlstatus = "ok") then
	iberrorissue = true
	return -1
end if
...
on fw_u_cht4browser.create
end on

on fw_u_cht4browser.destroy
end on
