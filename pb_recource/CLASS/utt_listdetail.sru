forward
global type utt_listdetail from u_subpage
end type
type dw_pagelist from u_dw within utt_listdetail
end type
type dw_pagedetail from u_dw within utt_listdetail
end type
type st_move from pf_u_splitbar_horizontal within utt_listdetail
end type
end forward
...
global utt_listdetail utt_listdetail
...
event ue_subpage_copyall;IF NOT tbTabpageSelected THEN EVENT ue_subpage_selected ()   // 처음 선택되었다. 자료의 재조회가 필요하다.
...
event resize;dw_pagelist.width = this.width - Long(PixelsToUnits(10, XPixelsToUnits!))
st_move.width = dw_pagelist.width
dw_pagedetail.width = dw_pagelist.width

dw_pagelist.height = st_move.y - dw_pagelist.y - Long(PixelsToUnits(2, XPixelsToUnits!))
dw_pagedetail.height = height - dw_pagelist.y - dw_pagelist.height - st_move.height - dw_pagedetail.of_dwheightminus1value() - Long(PixelsToUnits(12, XPixelsToUnits!))
end event
...
type ln_temptop from u_subpage`ln_temptop within utt_listdetail
end type

type ln_tempstart from u_subpage`ln_tempstart within utt_listdetail
end type

type ln_templeft from u_subpage`ln_templeft within utt_listdetail
end type

type ln_cond_start from u_subpage`ln_cond_start within utt_listdetail
end type

type ln_tempright from u_subpage`ln_tempright within utt_listdetail
end type

type ln_cond1_yline from u_subpage`ln_cond1_yline within utt_listdetail
end type

type ln_dw1_yline from u_subpage`ln_dw1_yline within utt_listdetail
end type

type ln_tempbutton from u_subpage`ln_tempbutton within utt_listdetail
end type
...
string bottomdragobject = "dw_pagedetail"
end type
