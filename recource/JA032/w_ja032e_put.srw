forward
global type w_ja032e_put from wt_vertdetail
end type
type st_port from pf_u_splitbar_horizontal within w_ja032e_put
end type
type dw_port from u_dw within w_ja032e_put
end type
type cb_recom from pf_u_commandbutton within w_ja032e_put
end type
type cb_syt0mg from pf_u_commandbutton within w_ja032e_put
end type
end forward
...
global w_ja032e_put w_ja032e_put
...
type lb_dirlist from wt_vertdetail`lb_dirlist within w_ja032e_put
end type

type ln_templeft from wt_vertdetail`ln_templeft within w_ja032e_put
end type

type ln_tempbuttom from wt_vertdetail`ln_tempbuttom within w_ja032e_put
end type

type ln_temptop from wt_vertdetail`ln_temptop within w_ja032e_put
end type

type ln_tempbutton from wt_vertdetail`ln_tempbutton within w_ja032e_put
end type

type ln_tempstart from wt_vertdetail`ln_tempstart within w_ja032e_put
end type

type ln_cond1_yline from wt_vertdetail`ln_cond1_yline within w_ja032e_put
end type

type ln_dw1_yline from wt_vertdetail`ln_dw1_yline within w_ja032e_put
end type

type ln_cond2_yline from wt_vertdetail`ln_cond2_yline within w_ja032e_put
end type

type ln_dw2_yline from wt_vertdetail`ln_dw2_yline within w_ja032e_put
end type

type ln_tempright from wt_vertdetail`ln_tempright within w_ja032e_put
end type

type uo_navi from wt_vertdetail`uo_navi within w_ja032e_put
end type

type ln_temptop_shadow from wt_vertdetail`ln_temptop_shadow within w_ja032e_put
end type

type st_windelaytime from wt_vertdetail`st_windelaytime within w_ja032e_put
end type

type st_top_rect from wt_vertdetail`st_top_rect within w_ja032e_put
end type

type p_close from wt_vertdetail`p_close within w_ja032e_put
end type

type p_excel from wt_vertdetail`p_excel within w_ja032e_put
end type

type p_print from wt_vertdetail`p_print within w_ja032e_put
end type

type p_delete from wt_vertdetail`p_delete within w_ja032e_put
end type

type p_update from wt_vertdetail`p_update within w_ja032e_put
end type

type p_input from wt_vertdetail`p_input within w_ja032e_put
end type

type p_retrieve from wt_vertdetail`p_retrieve within w_ja032e_put
end type

type p_clear from wt_vertdetail`p_clear within w_ja032e_put
end type

type p_copy from wt_vertdetail`p_copy within w_ja032e_put
end type

type dw_c from wt_vertdetail`dw_c within w_ja032e_put
...
type btn_update from wt_vertdetail`btn_update within w_ja032e_put
end type

type st_count from wt_vertdetail`st_count within w_ja032e_put
end type

type dw_list from wt_vertdetail`dw_list within w_ja032e_put
string dataobject = "d_ja032e1_put"
...
type dw_detail from wt_vertdetail`dw_detail within w_ja032e_put
integer y = 1236
integer height = 1528
string dataobject = "d_ja032e3_put"
...
ls_jm_cd = SQLCA.GETITEMSTRING (1) + ' P%'

f_microhelp (string (iRow) + ',' + string (ldt_mangi,'yyyymmdd') + '-' + ls_jm_cd + ',' + string (dw_port.object.jonga [ll_p]))
...
CHOOSE CASE dwo.name
	CASE 'tr_su'
		ll = dw_port.getrow ()
		IF	dw_port.object.min_jusu [ll]>0 And Object.hangsa_ga [row] > 0	Then
			Object.tr_su [row] = truncate (dw_port.object.mo_amt [ll] / Object.hangsa_ga [row] / 100, 0)
		End IF
END CHOOSE
end event
...
type st_move from wt_vertdetail`st_move within w_ja032e_put
...
integer taborder = 30
boolean bringtotop = true
string dataobject = "d_ja032e2_put"
boolean hscrollbar = true
...
FOR  ll =1  TO  ll_cnt
   IF f_num (dw_detail.object.tr_su [ll])>0 AND f_num(dw_detail.object.bid_ask [ll])>0  Then
      ...
      CHOOSE CASE LEFT (ls_jm_cd, 1)
         CASE '1'
            ls_jm_nm = f_replace (MID (ls_jm_cd, 2), ' P', '(1W) P')
         CASE '2'
            ls_jm_nm = f_replace (MID (ls_jm_cd, 2), ' P', '(2W) P')
         CASE '4'
            ls_jm_nm = f_replace (MID (ls_jm_cd, 2), ' P', '(4W) P')
         CASE '5'
            ls_jm_nm = f_replace (MID (ls_jm_cd, 2), ' P', '(5W) P')
         CASE ELSE
            ls_jm_nm = ls_jm_cd
      END CHOOSE
